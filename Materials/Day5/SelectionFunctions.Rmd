...
dist_raster[] = apply(dist_water_grid, 1, min))
...
layers <- map(rasters, as.data.frame) %>% 
  bind_cols %>% 
  rename(dist_rd = layer,
         dist_H2O = layer1)
...
predictions = raster::predict(object = rasters, model=m, fun=predict) #based on the scale of the linear predictors
