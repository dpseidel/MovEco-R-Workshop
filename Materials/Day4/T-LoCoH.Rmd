...
algo <- function(train.test, k.vals, s.max, num.s.vals, data.lxy, data) {
  ...
  for (k in 1:length(k.vals)) {
    ...
    trace[((k-1)*(num.s.vals+1))+z,1,2] <- current.k_val
    trace[((k-1)*(num.s.vals+1))+z,1,3] <- hull.mean
    trace[((k-1)*(num.s.vals+1))+z,1,4] <- new.postLike
    trace[((k-1)*(num.s.vals+1))+z,1,5] <- new.lnLike
    trace[((k-1)*(num.s.vals+1))+z,1,6] <- new.AIC
    trace[((k-1)*(num.s.vals+1))+z,1,7] <- new.BIC
