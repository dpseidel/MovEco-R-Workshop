...
```{r read.table_nastrings_colnames}
...
```{r read_table message=FALSE, warning=FALSE}
...
# here we can get a good sense of the missing values in the days column and average column. 

# for the first or last `n` lines of the data frame
# check out r help - shows us that the default argument is 10 lines
...
# pipes and select, multiple columns

# dropping columns, select/bracket
# select all columns except year


# filter v. subset
 # comma functions as "and" in the filter function
 # but and must be explicit in the subset function

# OR in filter # | is equivalent to "or"
...
```{r}
# demonstrate pipe from filter to select


# compared to intermediate objects
