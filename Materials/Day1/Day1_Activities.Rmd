...
compare. Mutate two new columns take the log of the original hare and lynx columns. 
...
take the value of a given population at time t and at it to the change in the population
...
### Task 4.3: Try putting together your simulation in a custom function taking the above arguments and 
looping over the discrete equations listed above. Your simulation function should include
each of the 3 steps: initialize, iterate, and output.
...
Use `geom_path` to create a phase plane diagram i.e a plot a of hare size vs linx size with various starting conditions.  
...
4. Introduce a saturating function into your functions for hare and lynx populations to reflect the search and 
handling time that limit the number of prey lynx can take in a given time step. 
...
Note that $h(x)$ is called a *functional response*, or handling time. Write out 
this as a new custom r fucntion. Adapt and rerun your simulation. 
